...
<script>
    ...
    $("#registrar").click(function (e) {
        ...
        if (($('#porcentaje_comision').val().trim() == '')) {
            ...
            $.post('<?php echo base_url(); ?>index.php/configuracion/CComisionRetiro/actualizar', $('#form_comision').serialize(), function (response) {
                if (response == 'fallo') {
                    bootbox.alert("Disculpe, no se pudo actualizar, consulte con su administrador", function () {
                    }).on('hidden.bs.modal', function (event) {
                        $("#porcentaje_comision").parent('div').addClass('has-error')
                        $("#porcentaje_comision").focus();
                    });
                }else {
                    bootbox.alert("Se actualizó con exito", function () {
                    }).on('hidden.bs.modal', function (event) {
                        url = '<?php echo base_url(); ?>index.php/configuracion/CComisionRetiro'
                        window.location = url
                    });
                }
