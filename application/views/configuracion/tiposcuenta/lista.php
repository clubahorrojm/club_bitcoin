      ...
      <div class="content-wrapper" style="min-height: 1156px;">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Tipos de Cuenta</li>
          </ol>
        </section>
            ...
            <div class="col-xs-12">
              ...
              <div class="box box-primary">
                <div class="box-header">
                  <h3 class="box-title" style="color:#3C8DBC">Listado de Tipos de Cuenta</h3>
                </div><!-- /.box-header -->
                <button role="button" class="btn btn-primary" style="font-weight: bold;font-size: 13px; color: white " id="enviar"  >
                    
                    &nbsp;<span class="glyphicon glyphicon-plus"></span>&nbsp;Agregar Tipo de Cuenta
                </button>
                <br/>
                <div class="box-body">
                  <table id="tab_tipocuentas" class="table table-bordered table-striped">
                    <thead>
                      <tr>
                      ...
                      <tbody >    
                            ...
                            <?php foreach ($listar as $tipocuenta) { ?>
                                <tr style="font-size: 16px;text-align: center" class="{% cycle 'impar' 'par' %}" >
                                    <td>
                                        <?php echo $i; ?>
                                    </td>
                                     <td>
                                        <?php echo $tipocuenta->descripcion; ?>
                                    </td>                                 
                                    <td style='text-align: center'>
									  <?php if ($tipocuenta->activo == 't') {?>
									  <input class='activar_desactivar' id='<?php echo $tipocuenta->id; ?>' type="checkbox" title='Desactivar el tipo de cuenta <?php echo $tipocuenta->id;?>' checked="checked"/>
									  <?php }else if ($tipocuenta->activo == 'f'){ ?>
									  <input class='activar_desactivar' id='<?php echo $tipocuenta->id; ?>' type="checkbox" title='Activar el tipo de cuenta <?php echo $tipocuenta->id;?>'/>
									  <?php } ?>
									</td>
									<td style='text-align: center'>
									   <a title="Editar" href="<?php echo base_url() ?>index.php/configuracion/CTiposCuenta/editar/<?= $tipocuenta->id; ?>"><i class="fa fa-pencil text-orange"></i></a>
									</td>
									<td style='text-align: center'>
									   <a data-toggle="modal" data-target="#myModal" class='levantar' id='<?php echo $tipocuenta->id; ?>' title="Borrar" ><i class="fa fa-trash text-black"></i></a>
									 </td>
                                </tr>
                                <?php $i++ ?>
                            <?php } ?>
...
<div class="modal" id="myModal" role="dialog">
    ...
    <script>
       ...
       var TTCuentas = $('#tab_tipocuentas').dataTable({
        "paging": true,
        "lengthChange": false,
        "autoWidth": false,
        "searching": true,
   ...
   $('.levantar').click(function(e){
    ...
    $('#procesar').click(function(e){
        ...
        $.post('<?php echo base_url(); ?>index.php/configuracion/CTiposCuenta/eliminar/' + id + '', function (response) {
	    if (response[0] == "e") {
                bootbox.alert("Disculpe, el registro que desea eliminar se encuentra asociada a 1 o más registros", function () {
                }).on('hidden.bs.modal', function (event) {
		 window.location = '<?php echo base_url(); ?>index.php/configuracion/CTiposCuenta'
                });
            } else {
	...
	url = '<?php echo base_url() ?>index.php/configuracion/CTiposCuenta/registrar';
	window.location = url;
    });
	...
	$("table#tab_tipocuentas").on('ifChanged', 'input.activar_desactivar', function (e) {
	    e.preventDefault();
		...
		bootbox.confirm("¿Desea "+accion+" este tipo de cuenta?", function(result) {
			if (result) {
				...
				$.post('<?php echo base_url(); ?>index.php/configuracion/CTiposCuenta/activar_desactivar/' + id, {'accion':accion}, function(response) {
					bootbox.alert("El Tipo de Cuenta fue "+mensaje+" exitosamente", function () {
					}).on('hidden.bs.modal', function (event) {
						location.reload();
					});
				})
