            ...
            <div class="col-xs-12">
              ...
              <div class="box box-primary">
                <div class="box-header">
                  <h3 class="box-title" style="color:#3C8DBC">Listado de Paises</h3>
                </div><!-- /.box-header -->
                <button role="button" class="btn btn-primary" style="font-weight: bold;font-size: 13px; color: white " id="enviar"  >
                    
                    &nbsp;<span class="glyphicon glyphicon-plus"></span>&nbsp;Agregar País
                </button>
                <br/>
                <div class="box-body">
                  <table id="tab_bancos" class="table table-bordered table-striped">
				    ...
				    <td style='text-align: center'>
				       <a title="Editar" href="<?php echo base_url() ?>index.php/administracion/CPaises/editar/<?= $bancos->id; ?>"><i class="fa fa-pencil text-orange"></i></a>
				    </td>
				    <td style='text-align: center'>
...
<div class="modal" id="myModal" role="dialog">
   ...
   $('.levantar').click(function(e){
	...
	//$('#myModal').modal('keyboard')
        $.post('<?php echo base_url(); ?>index.php/administracion/CPaises/eliminar/' + id + '', function (response) {
	    if (response[0] == "e") {
                bootbox.alert("Disculpe, el registro que desea eliminar se encuentra asociada a 1 o más registros", function () {
                }).on('hidden.bs.modal', function (event) {
		 window.location = '<?php echo base_url(); ?>index.php/configuracion/CPaises'
                });
            } else {
	...
	url = '<?php echo base_url() ?>index.php/administracion/CPaises/registrar';
	window.location = url;
    });
	...
	$("table#tab_bancos").on('ifChanged', 'input.activar_desactivar', function (e) {
		...
		bootbox.confirm("¿Desea "+accion+" este banco?", function(result) {
			if (result) {
				...
				$.post('<?php echo base_url(); ?>index.php/administracion/CPaises/activar_desactivar/' + id, {'accion':accion}, function(response) {
					bootbox.alert("La País fue "+mensaje+" exitosamente", function () {
					}).on('hidden.bs.modal', function (event) {
						location.reload();
					});
				})
