...
/**
 * Description of ModelsBancos
 *
 * @author Ing. Marcel Arcuri
 */
class MBancos extends CI_Model {
    //put your code here
    ...
    public function insertarBancos($datos){
        $result = $this->db->where('codigo =', $datos['codigo']);
        $result = $this->db->get('conf_bancos');
        
        $result2 = $this->db->where('descripcion =', $datos['descripcion']);
        $result2 = $this->db->get('conf_bancos');
        ...
        if ($result->num_rows() > 0) {
            ...
            $result = $this->db->insert("conf_bancos", $datos);
            return $result;
        }
    }
    
    // Metodo publico, para obterner la unidad de medida por id
    public function obtenerBancos($id){
        $this->db->where('id',$id);    
        $query = $this->db->get('conf_bancos');        
        if($query->num_rows()>0) return $query->result();
        else return $query->result();
   ...
   // Metodo publico, para actualizar un registro 
    public function actualizarBancos($datos) {
        $result = $this->db->where('codigo =', $datos['codigo']);
        ...
        if ($result->num_rows() > 0) {
            ...
            $result = $this->db->where('id', $datos['id']);
            $result = $this->db->update('conf_bancos', $datos);
            return $result;
        }
    }
    ...
    public function eliminarBancos($id) {
        ...
        if ($result->num_rows() > 0) {
            ...
            $result = $this->db->delete('conf_bancos', array('id'=>$id));
            return $result; 
        }
